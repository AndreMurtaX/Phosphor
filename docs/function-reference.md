...
| `crt_hideconsole() → num` | let go of the console window **this process owns**, so a windowed program launched from a file manager shows no console. `1` if it released one, `0` if there was none of its own — running from a terminal answers `0` and touches nothing, because that console belongs to the shell. Printing afterwards is safe: standard output that *was* a console is pointed at the null device, while output redirected to a file or a pipe keeps its redirection, which is how a windowed program still writes a debug log. Windows only; `0` on Unix, where the terminal is the user's |
| `crt_showconsole() → num` | make a **new** console and print to it again, after `crt_hideconsole` released one. `1` if it did, `0` otherwise. It does not bring the old window back — it cannot |
